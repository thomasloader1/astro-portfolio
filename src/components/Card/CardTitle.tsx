...
const CardTitle: FC<Props> = ({title, date, company,isCurrent, onSubCard}) => {
  return (
    <div className={`${!isCurrent && "mb-4"} ml-2`}>
        <h2 className={`text-2xl font-bold ${isCurrent ? "text-white" : "text-grey-primary"}`}>
            {title} 
            <div className="inline">
                <span className={`mx-2 ${isCurrent ? "text-grey-primary" : "text-orange-primary"} ${onSubCard && 'hidden'}`}>@</span>
                {company != null && <a className="hover:text-orange-primary" href={company.link}>{company.name}</a>}
            </div>
        </h2>
